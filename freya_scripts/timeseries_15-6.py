...
radar_sample_file = "3B-HHR-GIS.MS.MRG.3IMERG.20170913-S233000-E235959.asc"
#radar_sample_file = "3B-HHR-GIS.0913-S160000-E162959_con.asc"
radarpath = "/Users/freyamuir/Documents/Masters/MDiss/primary/max/20170913/asc/ascsample/"
...
cumulative_rainfall_raster_name = 'test12jun_rainfall_totals.asc'

# Name to write rainfall timeseries file names to
original_rainfall_spatial_timeseries_name = 'test12jun_rainfile_30min.txt'
hourly_spatial_rainfall_timeseries_name = 'test12jun_rainfile_1hour.txt'
weighted_uniform_hourly_rainfall_name = "test12jun_uniform_mean_rainfall.txt"

# Name to write the cropped radar file to (for use in the hydroindex.py tool)
cropped_test_radar_name = "test12jun_cropped_radar_test.asc"
...
def calculate_crop_coords(basin_header, radar_header):
    ...
    x0_basin_UTM, y0_basin_UTM = (x0_basin, y0_basin)#convert_OSGB36_to_UTM30
    print("basin UTM (x,y):", x0_basin_UTM, y0_basin_UTM)
    
    x0_radar_UTM, y0_radar_UTM = (x0_radar, y0_radar)#convert_OSGB36_to_UTM30
    print("radar UTM (x,y):", x0_radar_UTM, y0_radar_UTM)
    
    # Gives new vars to the nrows, ncols and cell size for both basin and radar
    ...
    # p is the difference in sizes of basin and radar
...
def extract_cropped_rain_data():
    ...
    for f in glob.iglob(radar_mult_source):
        print("")
        print("Filename:", f)
        print("Timestep:", f[-18:-16]+":"+f[-16:-14]+":"+f[-14:-12],"-",f[-10:-8]+":"+f[-8:-6]+":"+f[-6:-4])
        ...
        print("rawgrid:")
        print(cur_rawgrid)
        print("rawgrid shape:",cur_rawgrid.shape)

        # Crops the rainfall radar grid to the subset area
        ## Division by 10 is to scale the rainfall values within the radar back to mm/hr (stored in cm/hr)
        cur_croppedrain = cur_rawgrid[start_row:end_row, start_col:end_col]/10 
        print("cropped rain:")
        print(cur_croppedrain)
        print("cropped rain shape:")
    ...
    print("hourly mean shape (hours,cols):", hourly_mean.shape) # Checks by printing number of hours (rows) by grid cells (columns)
...
def write_sample_radar_img():
    ...
    """NOTE: cropped_radar_test file is cropped correctly to the cells that cover 
    the basin DEM (with the correct rainfall values), but the header is then written
    to the lower left corner of the DEM, shifting the entire cropped radar file 
    up and over to the right incorrectly. llx, lly should be (-98.6999997, 17.3999997).
    Fixed by reassigning the xll and yll in the header to the original radar lat-long (FM 13/06/18)"""
    ...
    print("RADAR HEADER with basin settings [ncols, nrows, xllcorner, yllcorner, cellsize, nodata]:")
    print(sampleradar_header)
    print(radar_header)
    xllsample = radar_header[2]+(start_col*radar_header[4]) # Column number multiplied by cell size gives longitude
    yllsample = (radar_header[3]+(end_row*radar_header[4]))*-1 # Row number multiplied by cell size gives latitude
    sampleradar_header[2] = xllsample
    sampleradar_header[3] = yllsample
...
#create_catchment_mean_rainfall()